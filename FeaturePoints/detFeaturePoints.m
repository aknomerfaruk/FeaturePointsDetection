function [V, CONT] = detFeaturePoints(canny,resize,landcont)
...
if (landcont==4) || (landcont==2)
    ...
    if ((exist('D','var')) && (exist('E','var')) && (exist('S','var')) && (exist('I','var')))
        ...
        col_middle = eucDistance(D(2), E(2));
...
if (landcont == 5)
    ...
    if ((S(1)~=-1) && (S(2)~=-1) && (newposS(1)==-1) && (newposS(2)==-1)) && ((I(1)~=-1) && (I(2)~=-1) && (newposI(1)==-1) && (newposI(2)==-1))
        S_Euclidiana  = eucDistance(S, I);
        dists = S_Euclidiana/2;
        de2 = dists/resize;
    end
    
    if ((S(1)~=-1) && (S(2)~=-1) && (newposS(1)~=--1) && (newposS(2)~=--1)) && ((I(1)~=-1) && (I(2)~=-1) && (newposI(1)~=-1) && (newposI(2)~=-1))
        S_Euclidiana  = eucDistance(newposS, newposI);
